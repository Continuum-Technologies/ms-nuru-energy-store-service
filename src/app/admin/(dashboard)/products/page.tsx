...
import { Plus, Package, FileEdit, CheckCircle2, PackageX } from "lucide-react";
  ...
  const columns: DataListColumn<ProductRow>[] = [
    {
      ...
      hideOnMobile: true,
      render: (row) => (
        <Link href={`/admin/products/${row.id}/edit`} className="font-medium text-foreground hover:text-brand-600">
          {row.name}
        </Link>
      ),
    },
    { key: "sku", header: "SKU", render: (row) => <span className="text-neutral-500">{row.sku}</span> },
    { key: "category", header: "Category", render: (row) => row.categoryName },
    { key: "price", header: "Price", render: (row) => formatKes(row.sellingPrice) },
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center justify-end gap-1">
          <Link href={`/admin/products/${row.id}/edit`} className={buttonVariants({ variant: "ghost", size: "sm" })}>
            Edit
          </Link>
